...
def struct_to_bio_dict(struct_ann_path, oversampling_rate):
    ...
    for doc_id in struct['annotated_docs']:
        bio_dict[doc_id] = create_doc_bio_annotations(struct['documents'][doc_id], oversampling_rate)
        
    return bio_dict


ner_tags = ['authors', 'study_type', 'arm_efficacy_metric', 'arm_efficacy_results', 'arm_dosage', 'arm_description']
def create_doc_bio_annotations(doc_struct, oversampling_rate=1, filters={}):
    bio_pars = []
...
def create_doc_bio_annotations_rd(doc_struct, filters={}):
    par_dicts = []
                    ...
                    for ac, dn in par_annotations:
                        if (ac == a_count or ac == '*') and (dn == d_num or dn == '*'):
                            ...
                            if k != default_trigger:
                                par_annotations[(ac, dn)]['labs'][start] = f'B-{k}'
                                for l_idx in range(start + 1, stop):
                                    par_annotations[(ac, dn)]['labs'][l_idx] = f'I-{k}'
                            else:
                                if 'desc_range' not in par_annotations[(ac, dn)]:
                                    par_annotations[(ac, dn)]['desc_range'] = []
                                par_annotations[(ac, dn)]['desc_range'].append((start, stop))
