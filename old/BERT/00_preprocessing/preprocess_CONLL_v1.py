...
output_path_1 = 'C:\\Users\\siban\\Desktop\\00_takeda\\00_data\\preprocessed_TEST.txt'
output_path_2 = 'C:\\Users\\siban\\Desktop\\00_takeda\\00_data\\preprocessed_all_TEST.txt'
...
#%% Open output files
fw_1 = codecs.open(output_path_1, 'w', 'utf-8')
fw_2 = codecs.open(output_path_2, 'w', 'utf-8')
...
for idx, row in data.iterrows():
    ...
    # Initialize token tags to ''
    par_tags_all = {x:[''] * len(par_tokens) for x in tgt_vars}
    ...
    for tgt_var in tgt_vars:
        ...
        for span in annot_spans:
            start, end = span
            par_tags_all[tgt_var][start] = 'B-' + tgt_var[0:6]
            if end == start + 1:
                continue
            for i in range(start+1, end):
                par_tags_all[tgt_var][i] = 'I-' + tgt_var[0:6]
        
        # Single par tags
        if annot_tgt_var != '':  # Skip empty fields
            # Iterate over spans in field
            for span in annot_spans:
                start, end = span
                par_tags[start] = 'B-' + tgt_var[0:6]
                if end == start + 1:
                    continue
                for i in range(start+1, end):
                    par_tags[i] = 'I-' + tgt_var[0:6]
        else:
            continue
            
    # Write output file 1
    for token, tag in zip(par_tokens, par_tags):
        fw_1.write(str(idx) + '\t' + token + '\t' + tag + '\n')
    fw_1.write('--------------------------------------------------------------\n')
    
    # Write output file 2
    for i, token in enumerate(par_tokens):
        fw_2.write(str(idx) + '\t' +
                   token + '\t' +
                   par_tags_all['title'][i] + '\t' +
                   par_tags_all['authors'][i] + '\t' +
                   par_tags_all['journal'][i] + '\t' +
                   par_tags_all['study_type'][i] + '\t' +
                   par_tags_all['arm_description'][i] + '\t' +
                   par_tags_all['arm_dosage'][i] + '\t' +
                   par_tags_all['arm_efficacy_metric'][i] + '\t' +
                   par_tags_all['arm_efficacy_results'][i] + '\n')
